...
export async function POST(req) {
  try {
    ...
    if (!isHomepageForm && !isContactPageForm) {
      throw new Error('Invalid form type');
    }
    ...
    // --- User confirmation (only for contact page) ---
    let userMailOptions = null;
    if (isContactPageForm) {
      const { name, email, message } = data;
      userMailOptions = {
        from: `"Neysa Website" <${process.env.EMAIL_USER}>`,
        to: email,
        subject: 'We received your message – Neysa',
        html: `
          <div style="font-family: Arial, sans-serif;">
            <h2 style="color: #861981;">Hello ${name},</h2>
            <p>Thank you for reaching out to Neysa. We have received your message and will get back to you within 24 hours.</p>
            <p><strong>Your message:</strong><br/>${message}</p>
            <br/>
            <p>Best regards,<br/>The Neysa Team</p>
          </div>
        `,
      };
    }
