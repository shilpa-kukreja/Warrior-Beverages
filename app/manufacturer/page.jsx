...
export default function ManufacturingPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Reset form
        setFormData({ name: "", email: "", phone: "", city: "", experience: "" });
      } else {
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-50">
        <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8 lg:py-24">
          {/* Manufacturer Profile Card – same as before */}
          <motion.div
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, margin: "-50px" }}
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-semibold text-gray-900">
                      Near Gatta Factory, Kherati Road,<br />
          ...
          >
            {[
              { label: "Established", value: "2020+", icon: Award },
              { label: "Distribution Network", value: "Pan-India", icon: Truck },
              { label: "Quality Certifications", value: "FSSAI, ISO", icon: Shield },
            ].map((stat, idx) => (
              <motion.div
                key={idx}
                whileHover={{ y: -6 }}
                className="rounded-xl border border-gray-100 bg-white p-6 text-center shadow-lg transition-all hover:shadow-xl"
              >
                <div className="inline-flex rounded-full bg-[#304869]/10 p-3 text-[#304869]">
                  <stat.icon className="h-6 w-6" />
                </div>
          ...
          <motion.div
            ...
            viewport={{ once: true, margin: "-50px" }}
            variants={fadeUp}
            className="mt-16 rounded-2xl bg-gradient-to-br from-[#304869] to-[#1a2a44] p-8 text-white shadow-xl sm:p-12"
          >
            <div className="grid gap-8 lg:grid-cols-2 lg:items-center">
              <div>
                ...
                <p className="mt-4 text-lg text-white/80">
                  Join our growing network of distributors and bring premium quality beverages to every corner of India. 
                  We offer competitive margins, marketing support, and a reliable supply chain.
                </p>
                <ul className="mt-6 space-y-3">
                  {[
                    "High-quality products from FSSAI-approved facility",
                    "Exclusive territory rights",
                    "Dedicated sales and marketing support",
                    "Flexible order quantities and payment terms",
                  ].map((item, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <BadgeCheck className="mt-0.5 h-5 w-5 flex-shrink-0 text-yellow-300" />
              ...
              {/* Distributor Interest Form – updated */}
              <div className="rounded-xl bg-white/10 p-6 backdrop-blur-sm">
                <h3 className="text-xl font-semibold text-white">Express Your Interest</h3>
                ...
                <form onSubmit={handleSubmit} className="mt-4 space-y-4">
                  <input
                    ...
                    className="w-full rounded-lg border border-white/20 bg-white/10 px-4 py-3 text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-yellow-300"
                    required
                  />
                  <input
                    type="email"
                  ...
                  <select
                    ...
                    value={formData.city}
                    onChange={handleChange}
                    className="w-full rounded-lg border border-white/20 bg-white/10 px-4 py-3 text-gray-900 placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-yellow-300"
                    required
                  >
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    placeholder="Tell us about your distribution experience (optional)"
                    rows="3"
                    className="w-full rounded-lg border border-white/20 bg-white/10 px-4 py-3 text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-yellow-300"
                  />
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="flex w-full items-center justify-center rounded-lg bg-yellow-300 px-6 py-3 font-bold text-[#304869] transition hover:bg-yellow-200 disabled:opacity-70"
                  >
                    {isLoading ? (
                      <>
          ...
          >
            <p className="text-gray-600">
              For bulk orders or partnership inquiries, reach out to us directly:
            </p>
            <div className="mt-4 flex flex-wrap items-center justify-center gap-6">
              <a
                href="mailto:connect@warriorbeverages.in"
                className="inline-flex items-center gap-2 rounded-full bg-[#304869]/10 px-6 py-3 text-sm font-medium text-[#304869] transition hover:bg-[#304869]/20"
              >
